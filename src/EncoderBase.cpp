...
void EncoderBase::init() {
  // initialize all the internal variables of Sensor to ensure a "smooth"
  // startup (without a 'jump' from zero)
  getSensorAngle();
...
void EncoderBase::update() {
  ...
  if (val < 0) { // sensor angles are strictly non-negative. Negative values are
...
EncoderBase::Angle EncoderBase::getMechanicalAngle() { return m_anglePrev; }

EncoderBase::Angle EncoderBase::getAngle() { return (float)m_fullRotations * M_PI * 2 + m_anglePrev; }

EncoderBase::PreciseAngle EncoderBase::getPreciseAngle() { return (double)m_fullRotations * (double)M_PI * 2 + (double)m_anglePrev; }

EncoderBase::Rotations EncoderBase::getFullRotations() { return m_fullRotations; }
