...
#include "foc/current_sensor/LowSideCurrentSensor.hpp"
...
LowSideCurrentSensor::LowSideCurrentSensor(float _shunt_resistor, float _gain, int _pinA, int _pinB, int _pinC) {
  pinA = _pinA;
  pinB = _pinB;
  pinC = _pinC;
...
LowSideCurrentSensor::LowSideCurrentSensor(float _mVpA, int _pinA, int _pinB, int _pinC) {
  pinA = _pinA;
  pinB = _pinB;
  pinC = _pinC;
...
void LowSideCurrentSensor::calibrateOffsets() {
  ...
  for (int i = 0; i < calibration_rounds; i++) {
    ...
    if (_isset(pinC))
      offset_ic += (_readADCVoltageLowSide(pinC, params));
    _delay(1);
  }
...
PhaseCurrent_s LowSideCurrentSensor::getPhaseCurrents() {
