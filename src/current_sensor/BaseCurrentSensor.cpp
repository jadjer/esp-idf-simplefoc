...
float BaseCurrentSensor::getDCCurrent(float motor_electrical_angle) {
...
DQCurrent_s BaseCurrentSensor::getFOCCurrents(float angle_el) {
  // read current phase currents
  PhaseCurrent_s current = getPhaseCurrents();
...
ABCurrent_s BaseCurrentSensor::getABCurrents(PhaseCurrent_s current) {
...
//   - function calculating Clarke transform of the phase currents
DQCurrent_s BaseCurrentSensor::getDQCurrents(ABCurrent_s current, float angle_el) {
  // calculate park transform
  float ct;
  float st;
...
*/
void BaseCurrentSensor::linkDriver(FOCDriver *_driver) {
  driver = _driver;
  // save the driver type for easier access
  driver_type = driver->type();
}

void BaseCurrentSensor::enable() {
  // nothing is done here, but you can override this function
};

void BaseCurrentSensor::disable() {
  // nothing is done here, but you can override this function
};
...
int BaseCurrentSensor::driverAlign(float voltage, bool modulation_centered) {
...
PhaseCurrent_s BaseCurrentSensor::readAverageCurrents(int N) {
...
// 3 - success but gains inverted
// 4 - success but pins reconfigured and gains inverted
int BaseCurrentSensor::alignBLDCDriver(float voltage, BLDCDriver *bldc_driver, bool modulation_centered) {
...
// 3 - success but gains inverted
// 4 - success but pins reconfigured and gains inverted
int BaseCurrentSensor::alignStepperDriver(float voltage, StepperDriver *stepper_driver, bool modulation_centered) {
