 ...
 */
    ...
    enum FOCMotorStatus : std::uint8_t {
        ...
        motor_error [[maybe_unused]] = 0x08,        //!< Motor is in error state (recoverable, e.g. overcurrent protection active)
    ...
    protected:
        ...
        pid::PIDController m_PIDCurrentQ{2, 300.0f, 0.0f, 0,
                                         12.0f};//!< parameter determining the q current PID config
        pid::PIDController m_PIDCurrentD{2, 300.0f, 0.0f, 0,
                                         12.0f};//!< parameter determining the d current PID config
        LowPassFilter m_LPFCurrentQ{
                0.005f};                                                                 //!<  parameter determining the current Low pass filter configuration
        LowPassFilter m_LPFCurrentD{
                0.005f};                                                                 //!<  parameter determining the current Low pass filter configuration
        pid::PIDController m_PIDVelocity{0.5f, 10.0f, 0.0f, 1000.0f,
                                         12.0f};   //!< parameter determining the velocity PID configuration
        pid::PIDController m_PIDAngle{20.0f, 0, 0, 0,
                                      20.0f};                                                   //!< parameter determining the position PID configuration
        LowPassFilter m_LPFVelocity{
                0.005f};                                                                  //!<  parameter determining the velocity Low pass filter configuration
        LowPassFilter m_LPFAngle{
                0.0};                                                                                   //!<  parameter determining the angle low pass filter configuration
